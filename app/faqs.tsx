...
import { aboutApp, privacyPolicy } from "@/constants/constants";

const PrivacyPolicy = () => {
  return (
    <ScrollView contentContainerStyle={styles.scrollContainer}>
      <View style={styles.container}>
        <Text style={styles.heading}>About Aviaxin Mobile Application </Text>
        <Text style={styles.subHeading}>Effective Date: [9/02/2024]</Text>
        {aboutApp?.map((policy, index) => (
          <View key={index} style={styles.termContainer}>
            <Text style={styles.title}>{policy.title}</Text>
            <Text style={styles.bodyText}>{policy.content}</Text>
            {policy.bulletPoints && (
              <View style={styles.restrictionsContainer}>
                {policy.bulletPoints.map((restriction, idx) => (
